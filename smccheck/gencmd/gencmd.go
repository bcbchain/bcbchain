...
func createSmcEx(transID, txID int64, callParam types.RPCInvokeCallParam) sdk.ISmartContract {
	...
	bcbgls.Mgr.SetValues(bcbgls.Values{bcbgls.SDKKey: smc}, func() {
		...
		tx := object.NewTx(smc, callParam.Tx.Note, callParam.Tx.GasLimit, callParam.GasLeft, callParam.TxHash, callParam.Sender)
		smc.SetTx(tx)
	})
...
func getBlock(transID, height int64) std.Block {
	...
	if err != nil {
		logger.Errorf(fmt.Sprintf("socket getBlock error: %s", err.Error()))
		return blockResult
	}

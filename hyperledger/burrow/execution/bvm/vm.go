...
import (
	...
	newSha3 "golang.org/x/crypto/sha3"
)
...
// Create a new BVM instance. Nonce is required to be globally unique (nearly almost surely) to avoid duplicate
// addresses for BVM created Accounts. In Burrow we use TxHash for this but a random nonce or sequence number could be
// used.
...
func (vm *VM) Debugf(format string, a ...interface{}) {
	...
	//fmt.Printf(format, a...)
	if vm.debugOpcodes {
		vm.logger.Debug(fmt.Sprintf(format, a...), "tag", "DebugOpcodes")
	}
}

func (vm *VM) fireCallEvent(eventSink EventSink, callType exec.CallType, errProvider errors.Provider, output *[]byte,
	callerAddress, calleeAddress crypto.BVMAddress, input []byte, value bn.Number, gas *uint64, errSink errors.Sink) {
	// fire the post call event (including exception if applicable)
	eventErr := eventSink.Call(&exec.CallEvent{
		CallType: callType,
		CallData: &exec.CallData{
			Caller: crypto.ToAddr(callerAddress),
			Callee: crypto.ToAddr(calleeAddress),
			Data:   input,
			Value:  value,
			Gas:    *gas,
		},
		Origin:     crypto.ToAddr(vm.origin),
...
func (vm *VM) fireTransferEvent(eventSink EventSink, st Interface, callerAddress, calleeAddress crypto.BVMAddress, value bn.Number, errSink errors.Sink, errProvider errors.Provider) {
	// fire the post transfer event (including exception if applicable)
	eventErr := eventSink.CallTransfer(&exec.TransferData{
		Token: st.GetToken(),
		From:  crypto.ToAddr(callerAddress),
		To:    crypto.ToAddr(calleeAddress),
		Value: value,
	}, errors.AsException(errProvider.Error()))
	errSink.PushError(eventErr)
}

// Initiate an BVM call against the provided state pushing events to eventSink. code should contain the BVM bytecode,
...
func (vm *VM) call(callState Interface, eventSink EventSink, caller, callee crypto.BVMAddress, code,
	input []byte, value bn.Number, gas *uint64, callType exec.CallType) (output []byte, err errors.CodedError) {
	...
	callState.PushError(vm.transfer(eventSink, callState, caller, callee, value, callState, callState))
	...
	if len(code) > 0 {
		...
		if err != nil {
			...
			callState.PushError(vm.transfer(eventSink, callState, callee, caller, value, callState, callState))
		}
	}
...
func (vm *VM) delegateCall(callState Interface, eventSink EventSink, caller, callee crypto.BVMAddress,
	code, input []byte, value bn.Number, gas *uint64,
...
// Executes the BVM code passed in the appropriate context
func (vm *VM) execute(callState Interface, eventSink EventSink, caller, callee crypto.BVMAddress,
	code, input []byte, value bn.Number, gas *uint64) (returnData []byte) {
	vm.Debugf("(%d) (%s) %s (code=%d) gas: %v (d) %X\n", vm.stackDepth, caller, callee, len(code), *gas, input)

	logger := vm.logger.With("bvm_nonce", vm.nonce)
	...
	for {
		...
		case BALANCE: // 0x31
			...
			stack.PushBigInt(balance.Value())
			vm.Debugf(" => %v (%s)\n", balance, address)

		case ORIGIN: // 0x32
			stack.PushAddress(vm.origin)
			vm.Debugf(" => %s\n", vm.origin)
		...
		case CALLVALUE: // 0x34
			stack.PushBigInt(value.Value())
			vm.Debugf(" => %v\n", value)

		case CALLDATALOAD: // 0x35
			offset := stack.Pop64()
			data := subSlice(input, offset, Word256Length, callState)
		...
		case GASPRICE_DEPRECATED: // 0x3A
			GasPrice := Int64ToWord256(vm.params.GasPrice)
			stack.Push(GasPrice)
			vm.Debugf(" => %X (GASPRICE IS DEPRECATED)\n", Zero256.Bytes())

		case DIFFICULTY_DEPRECATED: // 0x3A
			stack.Push64(0)
			vm.Debugf(" => %X (DIFFICULTY IS DEPRECATED)\n", Zero256.Bytes())

		case EXTCODESIZE: // 0x3B
			address := stack.PopAddress()
			useGasNegative(gas, GasGetAccount, callState)
			if callState.Exists(address) {
				code := callState.GetBVMCode(address)
				l := int64(len(code))
				stack.Push64(l)
				vm.Debugf(" => %d\n", l)
			} else {
				...
				vm.Debugf(" => returning code size of 1 to indicated existence of native contract at %s\n", address)
				stack.Push(One256)
			}
		case EXTCODECOPY: // 0x3C
			...
			if !callState.Exists(address) {
				if _, ok := registeredNativeContracts[address]; ok {
					vm.Debugf(" => attempted to copy native contract at %s but this is not supported\n", address)
		...
		case BLOCKHASH: // 0x40
			...
			BlockHeight := uint64(vm.params.BlockHeader.Height)
			if blockNumber == BlockHeight {
				blockHash := LeftPadWord256(vm.params.BlockHash)
				stack.Push(blockHash)
				vm.Debugf(" == 0x%X\n", blockHash.Bytes())
			} else if blockNumber > BlockHeight {
			...
			} else if BlockHeight-blockNumber > MaximumAllowedBlockLookBack {
				...
				useGasNegative(gas, GasBlockHash, callState)
		...
		case COINBASE: // 0x41
			coinbase := crypto.ToBVM(vm.params.BlockHeader.ProposerAddress)
			stack.Push(coinbase.Word256())
			vm.Debugf(" => 0x%X (NOT SUPPORTED)\n", stack.Peek().Bytes())

		case TIMESTAMP: // 0x42
			time := vm.params.BlockHeader.Time
			stack.Push64(time)
			vm.Debugf(" => 0x%X\n", time)

		case BLOCKHEIGHT: // 0x43
			number := uint64(vm.params.BlockHeader.Height)
		...
		case LOG0, LOG1, LOG2, LOG3, LOG4:
			n := int(op - LOG0)
			topics := make([]Word256, n)                         // 0 事件Hash ,其他是事件的参数值
			offset, size := stack.PopBigInt(), stack.PopBigInt() //堆栈的偏移量位置  堆栈大小
			for i := 0; i < n; i++ {
				topics[i] = stack.Pop()
			}
			data := memory.Read(offset, size)
			callState.PushError(eventSink.Log(&exec.LogEvent{
				Address: crypto.ToAddr(callee),
				Topics:  topics,
				Data:    data,
			}))
			vm.Debugf(" => T:%X D:%X\n", topics, data)
		...
		case CREATE, CREATE2: // 0xF0, 0xFB
			...
			var newAccount crypto.BVMAddress
			chainID := vm.params.BlockHeader.ChainID
			if op == CREATE {
				vm.sequence++
				addr := CalcContractAddr(crypto.ToAddr(caller), vm.nonce, chainID)
				newAccount = crypto.ToBVM(addr)
			} else if op == CREATE2 {
				salt := stack.Pop()
				privateKey := gocrypto.GenPrivKeyEd25519FromSecret(append(salt[:], callState.GetBVMCode(callee)...))
				newAccount = crypto.ToBVM(privateKey.PubKey().Address(chainID))
			}

			// Establish a frame in which the putative account exists
			childCallState := callState.NewCache()
			createAccount(childCallState, newAccount)

			// Run the input to get the contract code.
			// NOTE: no need to copy 'input' as per Call contract.
			ret, callErr := vm.Call(childCallState, eventSink, callee, newAccount, input, input, bn.NB(contractValue), gas)
			if callErr != nil {
				stack.Push(Zero256)
			...
			*gas -= gasLimit
			...
			if IsRegisteredNativeContract(address) {
				...
				if !callState.Exists(address) {
					...
					createAccount(callState, address)
					if callState.Error() != nil {
						continue
					}
				}
				...
				case CALL:
					childCallState = callState.NewCache()
					returnData, callErr = vm.call(childCallState, eventSink, callee, address, callState.GetBVMCode(address),
						args, value, &gasLimit, exec.CallTypeCall)

				case CALLCODE:
					childCallState = callState.NewCache()
					returnData, callErr = vm.call(childCallState, eventSink, callee, callee, callState.GetBVMCode(address),
						args, value, &gasLimit, exec.CallTypeCode)

				case DELEGATECALL:
					childCallState = callState.NewCache()
					returnData, callErr = vm.delegateCall(childCallState, eventSink, caller, callee,
						callState.GetBVMCode(address), args, value, &gasLimit, exec.CallTypeDelegate)
				...
				case STATICCALL:
					...
					returnData, callErr = vm.delegateCall(childCallState, NewLogFreeEventSink(eventSink),
						callee, address, callState.GetBVMCode(address), args, value, &gasLimit, exec.CallTypeStatic)
			...
			if callErr != nil {
				...
				// returnData is empty, but since BVM expects retSize to be respected this will
		...
		case SELFDESTRUCT: // 0xFF
			...
			if !callState.Exists(receiver) {
				...
				useGasNegative(gas, GasCreateAccount, callState)
				createAccount(callState, receiver)
				if callState.Error() != nil {
					continue
				}
			}
			...
			vm.fireTransferEvent(eventSink, callState, callee, receiver, balance, callState, callState)
			vm.Debugf(" => (%s) %v\n", receiver[:4], balance)
			return nil
...
func createAccount(st Interface, address crypto.BVMAddress) {
	if IsRegisteredNativeContract(address) {
...
// (true if slice was possible). If the subSlice
...
//func (vm *VM) transfer(st Interface, eventSink EventSink, from, to crypto.BVMAddress, amount bn.Number) errors.CodedError {
func (vm *VM) transfer(eventSink EventSink, st Interface, from, to crypto.BVMAddress, amount bn.Number, errSink errors.Sink, errProvider errors.Provider) errors.CodedError {

	if amount.IsEqualI(0) {
		return nil
	}
	if st.GetBalance(from).IsLessThan(amount) {
	...
	vm.fireTransferEvent(eventSink, st, from, to, amount, errSink, errProvider)

	return nil
}

// Dump the byteCode being sent to the BVM in the current working directory
func dumpTokens(nonce []byte, caller, callee crypto.BVMAddress, code []byte) {
...
func CalcContractAddr(senderAddr gocrypto.Address, nonce []byte, chainID string) gocrypto.Address {
	...
	mainChainID := crypto.GetMainChainID(chainID)
	hasherSHA3256 := newSha3.New256()
	...
	chainName := crypto.GetChainName(chainID)
	if chainName != "" {

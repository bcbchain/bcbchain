...
func TestPacker(t *testing.T) {
	for _, test := range []struct {
		...
		t.Log(test.args)
		if output, _, err := EncodeFunctionCall(test.ABI, test.name, log.NewGiLogger("/dev/stdout", ""), test.args...); err != nil {
...
func TestUnpacker(t *testing.T) {
	for _, test := range []struct {
		...
		//t.Log(test.name)
		t.Log(test.packed)
		_, err := test.bvm.unpack(test.packed, 0, test.data)
		if err != nil {
			t.Errorf("Unpacker failed: %v", err)
		}

...
import (
	...
	. "github.com/bcbchain/bcbchain/hyperledger/burrow/execution/bvm/asm/bc"
...
func NewTestState(st acmstate.ReaderWriter, blockHashGetter func(uint64) []byte) *testState {
	bvmState := NewState(st, blockHashGetter)
	return &testState{
		State:             bvmState,
		BlockHashProvider: bvmState.GetBlockHash,
	}
}
...
func newAccount(st Interface, name string) crypto.BVMAddress {
	address := newAddress(name)
	st.CreateAccount(address)
	return address
}

func makeAccountWithCode(st Interface, name string, code []byte) crypto.BVMAddress {
	address := newAddress(name)
	st.CreateAccount(address)
...
func TestCreate2(t *testing.T) {
	...
	output, err := ourVm.Call(cache, NewBcEventSink(logger, &tags), caller, callee, cache.GetBVMCode(callee), []byte{}, binary.N0, &gas)

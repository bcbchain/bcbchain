...
type Cache struct {
	...
	accounts  map[crypto.BVMAddress]*accountInfo
	readonly  bool
}

func (cache *Cache) GetToken() goCrypto.Address {
	if cache.tokenAddr == "" {
		cache.tokenAddr = cache.backend.GetToken()
	}
	return cache.tokenAddr
}

func (cache *Cache) SetToken(token goCrypto.Address) {
	cache.tokenAddr = token
}
...
func NewCache(backend Reader, options ...CacheOption) *Cache {
	cache := &Cache{
		backend:  backend,
		accounts: make(map[crypto.BVMAddress]*accountInfo),
	}
...
func (cache *Cache) GetAccount(address crypto.BVMAddress) (*acm.Account, error) {
...
func (cache *Cache) UpdateAccount(account *acm.Account) error {
	...
	if cache.readonly {
		return errors.ErrorCodef(errors.ErrorCodeIllegalWrite,
			"UpdateAccount called in a read-only context on account %v", account.Address)
	}
	...
	if accInfo.removed {
		return errors.ErrorCodef(errors.ErrorCodeIllegalWrite, "UpdateAccount on a removed account: %s", account.Address)
	}
...
func (cache *Cache) RemoveAccount(address crypto.BVMAddress) error {
	if cache.readonly {
...
func (cache *Cache) GetStorage(address crypto.BVMAddress, key binary.Word256) ([]byte, error) {
	accInfo, err := cache.get(address)
	if err != nil {
		return []byte{}, err
	}
...
func (cache *Cache) SetStorage(address crypto.BVMAddress, key binary.Word256, value []byte) error {
	if cache.readonly {
		return errors.ErrorCodef(errors.ErrorCodeIllegalWrite,
			"SetStorage called in a read-only context on account %v", address)
	}
	accInfo, err := cache.get(address)
	if accInfo == nil || accInfo.account == nil {
		return errors.ErrorCodef(errors.ErrorCodeIllegalWrite,
...
func (cache *Cache) IterateCachedStorage(address crypto.BVMAddress,
...
func (cache *Cache) Sync(st Writer) error {
	...
	cache.Lock()
	defer cache.Unlock()
	var addresses crypto.BVMAddresses
...
func (cache *Cache) Reset(backend Reader) {
	...
	cache.accounts = make(map[crypto.BVMAddress]*accountInfo, len(cache.accounts))
}
...
func (cache *Cache) get(address crypto.BVMAddress) (*accountInfo, error) {
	cache.RLock()
	accInfo := cache.accounts[address]
	cache.RUnlock()
	if accInfo == nil {

...
// Get a Word160 embedded a Word256 and padded on the left (as it is for account addresses in BVM)
...
func LeftPadWord256ForString(str string) string {
	length := 64 - len(str)
	for i := 0; i < length; i++ {
		str = "0" + str
	}
...
func (ws Words256) String() string {
	var byt bytes.Buffer
	byt.WriteString("[")
	length := len(ws)
	for idx, w := range ws {
		byt.WriteString(w.String())
		if idx < length {
			byt.WriteString(",")
		}
	}

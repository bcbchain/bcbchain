...
func TestReflectTagged_Keys(t *testing.T) {
	...
	assert.Equal(t, []string{"Foo", "Bar", "Baz", "BVMAddress", "Indices"}, rt.Keys())
}
...
func TestReflectTagged_Get(t *testing.T) {
	tt := testTaggable{
		...
		Address: crypto.BVMAddress{1, 2, 3},
		Indices: []int{5, 7, 9},
	}
...
func TestExplicitFields(t *testing.T) {
	...
	rt, err := ReflectTags(&tt, "Foo", "BVMAddress")
	require.NoError(t, err)
	...
	_, err = ReflectTags(&tt, "Foo", "BVMAddress", "Balloons")
	require.Error(t, err)
}

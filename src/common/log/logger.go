...
// NewGiLogger returns a logger that encodes msg and keyvals to the Writer,
// or encodes fmtStr and vals to the writer
func NewGiLogger(path, module string) Loggerf {
	return &giLogger{
		path:             path,
		module:           module,
...
func (log *giLogger) genLogText(level, msg string, keyvals []interface{}) (logText string) {
	var kvs []interface{}
	var tag string
...
func (log *giLogger) genLogTextEx(level, fmtStr string, vals []interface{}) (logText string) {
	logText = fmt.Sprintf(
		"%v[%v][%-5v] ",
		time.Now().UTC().Format(DATETIMEFORMAT),
		log.timeZone,
	...
	for {
		if len(log.cf) > 0 {
			...
			if li := <-log.cf; li.fmt == false {
				logText = log.genLogText(li.level, li.msg, li.keyvals)
			} else {
				logText = log.genLogTextEx(li.level, li.fmtStr, li.vals)
			}
...
func (log *giLogger) Log(level, msg string, keyvals ...interface{}) {
	if log.isOutputAsync {
		...
		logText := log.genLogText(level, msg, keyvals)
...
func (log *giLogger) LogEx(level, fmtStr string, vals ...interface{}) {
	if log.isOutputAsync {
		...
		logText := log.genLogTextEx(level, fmtStr, vals)
		log.flushMutex(bytes.NewBuffer([]byte(logText)))
	}
}

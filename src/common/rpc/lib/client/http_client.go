...
func makeHTTPSClient(remoteAddr string, pool *x509.CertPool, disableKeepAlive bool, timeout time.Duration) (string, *http.Client) {
	...
	return remoteAddr, &http.Client{Transport: tr, Timeout: timeout}
}
...
func NewJSONRPCClientEx(remote, certFile string, disableKeepAlive bool) *JSONRPCClient {
	return NewJSONRPCClientExWithTimeout(remote, certFile, disableKeepAlive, 120)
}

func NewJSONRPCClientExWithTimeout(remote, certFile string, disableKeepAlive bool, timeout int) *JSONRPCClient {
	...
	address, client := makeHTTPSClient(remote, pool, disableKeepAlive, time.Duration(timeout)*time.Second)

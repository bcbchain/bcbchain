...
type Cache struct {
	...
	accounts  map[crypto.EVMAddress]*accountInfo
	readonly  bool
}
...
func NewCache(backend Reader, options ...CacheOption) *Cache {
	cache := &Cache{
		backend:  backend,
		accounts: make(map[crypto.EVMAddress]*accountInfo),
	}
...
func (cache *Cache) GetAccount(address crypto.EVMAddress) (*acm.Account, error) {
...
func (cache *Cache) RemoveAccount(address crypto.EVMAddress) error {
	if cache.readonly {
...
func (cache *Cache) GetStorage(address crypto.EVMAddress, key binary.Word256) ([]byte, error) {
	accInfo, err := cache.get(address)
	if err != nil {
		return []byte{}, err
	}
...
func (cache *Cache) SetStorage(address crypto.EVMAddress, key binary.Word256, value []byte) error {
	if cache.readonly {
		return errors.ErrorCodef(errors.ErrorCodeIllegalWrite,
...
func (cache *Cache) IterateCachedStorage(address crypto.EVMAddress,
...
func (cache *Cache) Sync(st Writer) error {
	...
	cache.Lock()
	defer cache.Unlock()
	var addresses crypto.EVMAddresses
...
func (cache *Cache) Reset(backend Reader) {
	...
	cache.accounts = make(map[crypto.EVMAddress]*accountInfo, len(cache.accounts))
}
...
func (cache *Cache) get(address crypto.EVMAddress) (*accountInfo, error) {
	cache.RLock()
	accInfo := cache.accounts[address]
	cache.RUnlock()
	if accInfo == nil {

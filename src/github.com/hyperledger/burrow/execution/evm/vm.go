...
// used.
func NewVM(params Params, origin crypto.EVMAddress, nonce []byte, logger log.Logger, options ...func(*VM)) *VM {
	vm := &VM{
...
func (vm *VM) fireCallEvent(eventSink EventSink, callType exec.CallType, errProvider errors.Provider, output *[]byte,
	callerAddress, calleeAddress crypto.EVMAddress, input []byte, value bn.Number, gas *uint64, errSink errors.Sink) {
...
func (vm *VM) fireTransferEvent(eventSink EventSink, st Interface, callerAddress, calleeAddress crypto.EVMAddress, value bn.Number, errSink errors.Sink, errProvider errors.Provider) {
...
func (vm *VM) Call(callState Interface, eventSink EventSink, caller, callee crypto.EVMAddress, code,
...
func (vm *VM) delegateCall(callState Interface, eventSink EventSink, caller, callee crypto.EVMAddress,
...
func (vm *VM) execute(callState Interface, eventSink EventSink, caller, callee crypto.EVMAddress,
	...
	for {
		...
		case COINBASE: // 0x41
			coinbase := crypto.ToEVM(vm.params.BlockHeader.ProposerAddress)
		...
		case CREATE, CREATE2: // 0xF0, 0xFB
			...
			var newAccount crypto.EVMAddress
			chainID := vm.params.BlockHeader.ChainID
			if op == CREATE {
				vm.sequence++
				newInt, err := strconv.Atoi(string(vm.nonce))
				if err != nil {
					vm.logger.Debug("evm: ", "nonce conversion failure: ", err)
					return
				}
				addr := CalcContractAddr(crypto.ToAddr(caller), uint64(newInt), chainID)
				newAccount = crypto.ToEVM(addr)
			} else if op == CREATE2 {
				salt := stack.Pop()
				privateKey := gocrypto.GenPrivKeyEd25519FromSecret(append(salt[:], callState.GetEVMCode(callee)...))
				newAccount = crypto.ToEVM(privateKey.PubKey().Address(chainID))
			}
...
func createAccount(st Interface, address crypto.EVMAddress) {
	if IsRegisteredNativeContract(address) {
...
//func (vm *VM) transfer(st Interface, eventSink EventSink, from, to crypto.EVMAddress, amount bn.Number) errors.CodedError {
func (vm *VM) transfer(eventSink EventSink, st Interface, from, to crypto.EVMAddress, amount bn.Number, errSink errors.Sink, errProvider errors.Provider) errors.CodedError {
...
func dumpTokens(nonce []byte, caller, callee crypto.EVMAddress, code []byte) {
...
func CalcContractAddr(senderAddr gocrypto.Address, nonce uint64, chainID string) gocrypto.Address {

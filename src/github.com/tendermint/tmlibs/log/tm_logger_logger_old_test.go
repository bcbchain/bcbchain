...
func BenchmarkTMLoggerSimple(b *testing.B) {
	benchmarkRunner(b, log.NewOldTMLogger(ioutil.Discard), baseInfoMessage)
}

func BenchmarkTMLoggerContextual(b *testing.B) {
	benchmarkRunner(b, log.NewOldTMLogger(ioutil.Discard), withInfoMessage)
}

...
func (cli *grpcClient) QueryExAsync(params types.RequestQueryEx) *ReqRes {
	req := types.ToRequestQueryEx(params)
	res, err := cli.client.QueryEx(context.Background(), req.GetQueryEx(), grpc.FailFast(true))
	if err != nil {
		cli.StopForError(err)
	}
	return cli.finishAsyncCall(req, &types.Response{&types.Response_QueryEx{res}})
}
...
func (cli *grpcClient) CleanDataAsync() *ReqRes {
	req := types.ToRequestCleanData()
	res, err := cli.client.CleanData(context.Background(), req.GetCleanData(), grpc.FailFast(true))
	if err != nil {
		cli.StopForError(err)
	}
	return cli.finishAsyncCall(req, &types.Response{&types.Response_CleanData{res}})
}
...
func (cli *grpcClient) QueryExSync(req types.RequestQueryEx) (*types.ResponseQueryEx, error) {
	reqres := cli.QueryExAsync(req)
	return reqres.Response.GetQueryEx(), cli.Error()
}
...
func (cli *grpcClient) CleanDataSync() (*types.ResponseCleanData, error) {
	reqres := cli.CleanDataAsync()
	return reqres.Response.GetCleanData(), cli.Error()
}

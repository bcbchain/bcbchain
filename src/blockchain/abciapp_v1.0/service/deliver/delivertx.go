...
import (
	...
	bctypes "blockchain/abciapp_v1.0/types"
...
func (conn *DeliverConnection) runDeliverBCTx(tx []byte, bFailed bool, fromAddr smc.Address, transaction bctx.Transaction, connV2 *deliver.AppDeliver) (resDeliverTx types.ResponseDeliverTx) {
	...
	if !bFailed {
		nonceBuffer, err := conn.stateDB.SetAccountNonce(smc.Address(fromAddr), transaction.Nonce)
		if err != nil {
			bFailed = true
		...
		if connV2 != nil {
			statedbhelper.CommitTx2V1(transID, nonceBuffer)
		}
	}
	...
	if bcerr.ErrorCode != bcerrors.ErrCodeOK {
		...
		txState.RollbackTx()
		conn.logger.Error("docker invoke error.....", "error", err)
		resDeliverTx = types.ResponseDeliverTx{
	...
	stateTx, txBuffer := txState.CommitTx()
	if connV2 != nil {
...
func (conn *DeliverConnection) runDeliverBCTxEx(tx []byte,
	...
	res, txBuffer := connV2.RunDeliverTx(tx, tx2, pubKey)

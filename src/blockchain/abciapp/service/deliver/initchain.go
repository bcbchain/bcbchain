...
import (
	...
	abcicommon "blockchain/abciapp/common"
)
...
func (app *AppDeliver) initChain(req abci.RequestInitChain) (response abci.ResponseInitChain) {
	...
	statedbhelper.SetChainID(req.ChainId)
	...
	appState := statedbhelper.GetWorldAppState(transID, 0)
	...
	r := adapterIns.InvokeTx(header, transID, 1, addr, tx, nil, nil, nil)

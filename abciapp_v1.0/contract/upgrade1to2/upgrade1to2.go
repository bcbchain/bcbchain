...
func (u *Upgrade1to2) Upgrade(conInfoList string) (chainVersion string, smcErr smc.Error) {
	...
	err := u.buildAndSaveNewContract(conInfoList, transID, txID)
	if err != nil {
		smcErr.ErrorCode = bcerrors.ErrCodeInterContractsRuntimeError
...
func (u *Upgrade1to2) buildAndSaveNewContract(conInfoList string, transID, txID int64) error {
	...
	for _, v := range *v2Contracts {
		...
		buildResult := builder.BuildContract(transID, txID, std.ContractMeta{
			Name:         v.Name,
			ContractAddr: addr,
			OrgID:        genesisOrgID,
			Version:      v.Version,
		...
		u.setOrganizationToV2Cache(transID, txID, &u.GenesisOrg)
		...
		u.setContract(&contract)
		u.setContractToV2Cache(transID, txID, &contract)
		...
		u.setContractMetaToV2Cache(transID, txID, &meta)
		...
		u.setContractVersionListToV2Cache(transID, txID, genesisOrgID, &versionList)
		if contract.Name == "token-basic" {
